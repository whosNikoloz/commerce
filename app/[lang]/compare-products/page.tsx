  ...
  const getFacetGroups = () => {
    ...
    products.forEach((product, index) => {
      product.productFacetValues.forEach((facet) => {
        const facetName = facet.facetName || "სხვა"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400">იტვირთება...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">პროდუქტების შედარება</h1>
          <p className="text-slate-600 dark:text-slate-400 mb-8">შედარების დასაწყებად გთხოვთ დაამატოთ პროდუქტები</p>
          <Button variant="outline" onClick={() => router.push(`/${currentLang}`)}>
  ...
  return (
    <div className="min-h-screen  py-12 px-4">
      <div className="container mx-auto max-w-7xl">
        {/* Header */}
        <div className="mb-10">
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white mb-6">პროდუქტების შედარება</h1>
          <div className="flex flex-wrap gap-3">
            ...
            >
              {showDifferencesOnly ? "ყველას ჩვენება" : "განსხვავებები"}
            </Button>
            <Button className="rounded-lg bg-transparent" size="sm" variant="outline" onClick={handleShare}>
              <Share2 className="h-4 w-4 mr-2" />
              გაზიარება
            </Button>
            <Button className="rounded-lg bg-transparent" size="sm" variant="outline" onClick={handleClearAll}>
              გასუფთავება
            </Button>
          </div>
        </div>
            ...
            <table className="w-full">
              <tbody>
                ...
                <CompareRow
                  label="ფასი"
                  showDifferencesOnly={showDifferencesOnly}
                  values={products.map((p) => [formatPrice(p.discountPrice || p.price)])}
                />
    ...
    <div className="flex-shrink-0 w-52 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-lg transition-all duration-300 group">
      ...
      <div className="relative h-40 bg-slate-100 dark:bg-slate-800 overflow-hidden">
        <Image
          ...
          src={resolveImageUrl(product.images?.[0]) || "/placeholder.svg?height=160&width=208"}
        />
        <button
      ...
      <div className="p-3.5">
        ...
        <div className="space-y-1.5 text-xs">
          <div className="flex justify-between">
            <span className="text-slate-600 dark:text-slate-400">ბრენდი:</span>
            <span className="font-medium text-slate-900 dark:text-slate-100">{product.brand?.name || "-"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-600 dark:text-slate-400">კატეგორია:</span>
            <span className="font-medium text-slate-900 dark:text-slate-100">{product.category?.name || "-"}</span>
          </div>
        </div>
      </div>
    </div>
...
function CompareRow({ label, values, showDifferencesOnly }: CompareRowProps) {
  const areAllSame = values.every((v) => v.join(",") === values[0].join(","))
      ...
      {values.map((val, index) => (
        <td
          ...
          className={cn(
            ...
            !areAllSame && val[0] !== "-" && "font-semibold text-blue-600 dark:text-blue-400",
          )}
        >
          <div className="flex items-start gap-2">
            ...
            <span className="text-slate-700 dark:text-slate-300">{val.join(", ")}</span>
          </div>
        </td>
      ))}
    </tr>
