                ...
                <Card className="col-span-4 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:shadow-md">
                    ...
                    <CardContent className="pl-2">
                        <div className="space-y-6">
                            {[
                                { id: "#1234", name: "John Doe", product: "Wireless Headphones", price: "$299.99", color: "from-blue-500 to-blue-600" },
                                { id: "#1235", name: "Jane Smith", product: "Smart Watch", price: "$149.99", color: "from-purple-500 to-purple-600" },
                                { id: "#1236", name: "Bob Johnson", product: "Coffee Maker", price: "$79.99", color: "from-orange-500 to-orange-600" },
                            ].map((order, index) => (
                                <div key={order.id} className="flex items-center">
                                    <div className={`w-10 h-10 bg-gradient-to-br ${order.color} rounded-full flex items-center justify-center`}>
                                        <span className="text-xs font-medium text-white">#{index + 1}</span>
                                    </div>
                                    <div className="ml-4 space-y-1">
                                        <p className="text-sm font-semibold text-slate-900 dark:text-white">Order {order.id}</p>
                                        <p className="text-sm text-slate-500 dark:text-slate-400">{order.name} – {order.product}</p>
                                    </div>
                                    <div className="ml-auto font-medium text-emerald-600 dark:text-emerald-400">+{order.price}</div>
                                </div>
                            ))}
                        </div>
                    </CardContent>
