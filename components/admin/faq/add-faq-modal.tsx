...
export default function AddFaqModal({
    ...
    useEffect(() => {
        if (!isOpen) {
            setQ(""); setA(""); setActive(true); setFeatured(false);
        }
    }, [isOpen]);

    const handleSave = async () => {
        if (!q.trim() || !a.trim()) { toast.error("შეავსე ველები"); return; }
        setLoading(true);
        try {
    ...
    return (
        <>
            {children ? (
                ...
                <Button size="sm" variant="default" onClick={onOpen}>+ Add FAQ</Button>
            )}
                ...
                <ModalContent className="bg-brand-muted dark:bg-brand-muteddark">
                    <>
                        <ModalHeader className="flex flex-col items-center gap-1 pb-2">
                            <h2 className="text-2xl font-bold dark:text-text-lightdark text-text-light">FAQ-ის დამატება</h2>
                        </ModalHeader>
