...
import { TriangleAlert, GripVertical, Plus, RefreshCw, Eye, EyeOff, SplitSquareVerticalIcon, Star } from "lucide-react";
...
function RowDraggable({ faq, children, isDragging }: { faq: FAQModel; children: React.ReactNode; isDragging: boolean }) {
    const { attributes, listeners, setNodeRef, transform, transition, isDragging: dragging } = useSortable({ id: faq.id });
    const style = {
...
export function FaqsTable({ initialFaqs }: { initialFaqs: FAQModel[] }) {
    ...
    async function saveOrder() {
        try {
            ...
            await Promise.all(
                changed.map(f => updateFaq({ ...f, orderNum: f.orderNum ?? (faqs.findIndex(x => x.id === f.id) + 1) }))
            );
    ...
    const filtered = useMemo(() => {
        ...
        return faqs
            .filter(f => (q ? (f.question ?? "").toLowerCase().includes(q) || (f.answer ?? "").toLowerCase().includes(q) : true))
    ...
    const handleUpdate = async (id: string, q: string, a: string, isActive: boolean, isFeatured: boolean) => {
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <div className="flex flex-1 items-center gap-3">
                            <Input
                                ...
                                startContent={<svg width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="m21 20l-4.35-4.35A7.5 7.5 0 1 0 9 16.5a7.47 7.47 0 0 0 4.65-1.6L18 19l3 1zM4.5 9A4.5 4.5 0 1 1 9 13.5A4.5 4.5 0 0 1 4.5 9"></path></svg>}
                            />
                            <div className="hidden md:flex items-center gap-5">
                                <div className="flex items-center gap-2">
                                    <Switch checked={onlyActive} onCheckedChange={setOnlyActive} className="data-[state=checked]:bg-blue-600" />
                                    {onlyActive ? (
                                    ...
                                    <Switch checked={onlyFeatured} onCheckedChange={setOnlyFeatured} className="data-[state=checked]:bg-blue-600" />
                                    <Star className={`h-3.5 w-3.5 ${onlyFeatured ? "fill-white text-white" : "text-slate-400"}`} />
                                        ...
                                        {filtered.map((faq, idx) => (
                                            <RowDraggable key={faq.id} faq={faq} isDragging={draggingId === faq.id}>
                                                ...
                                                <TableCell className="align-top max-w-[560px]">
                                                    <div className="line-clamp-2 text-slate-700 dark:text-slate-300">{faq.answer}</div>
                                                </TableCell>
                                                ...
                                                <TableCell className="align-top">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        />
                                                        <Star className={`h-3.5 w-3.5 ${!!faq.isFeatured ? "fill-white text-white" : "text-slate-400"}`} />
                                                    </div>
                                                </TableCell>
                                                ...
                                                <TableCell className="align-top text-right">
                                                    <div className="flex justify-end gap-2">
                                                        ...
                                                        <Button
                                                            variant="destructive"
                                                            size="sm"
                                                            onClick={() => { setDeleteTarget(faq); setDeleteOpen(true); }}
                                                        >
                                                            Delete
                                                        </Button>
                                                    </div>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {deleteTarget ? (
                                <>წაიშლება: <span className="font-semibold">{deleteTarget.question}</span>. ქმედება შეუქცევადია.</>
                            ) : "ქმედება შეუქცევადია."}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel disabled={deleting} onClick={() => { setDeleteOpen(false); setDeleteTarget(null); }}>
                            გაუქმება
                        </AlertDialogCancel>
                        <AlertDialogAction
