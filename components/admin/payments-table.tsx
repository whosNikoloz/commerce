  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card className="border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 backdrop-blur">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-900 dark:text-slate-100">Transaction Fees</CardTitle>
            <CreditCard className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">${totalFees.toFixed(2)}</div>
            <p className="text-xs text-muted-foreground text-slate-500 dark:text-slate-400">
        ...
        <Card className="border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 backdrop-blur">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-900 dark:text-slate-100">Failed Payments</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground text-slate-500 dark:text-slate-400">
              {((transactions.filter((t) => t.status === "failed").length / transactions.length) * 100).toFixed(1)}%
              failure rate
            </p>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="methods" className="space-y-4">
        <TabsList>
          <TabsTrigger value="methods" className="text-slate-900 dark:text-slate-100">
            Payment Methods
          </TabsTrigger>
          <TabsTrigger value="transactions" className="text-slate-900 dark:text-slate-100">
            Transactions
          </TabsTrigger>
        </TabsList>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-slate-900 dark:text-slate-100">Transaction Fee</TableHead>
                    <TableHead className="text-slate-900 dark:text-slate-100">Description</TableHead>
                ...
                <TableBody>
                  {methods.map((method) => (
                    <TableRow key={method.id} className="border-slate-200 dark:border-slate-700">
                      ...
                      <TableCell>
                        <Badge variant="outline" className="text-slate-900 dark:text-slate-100">
          ...
          <Card className="border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 backdrop-blur">
            ...
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-slate-900 dark:text-slate-100">Transaction ID</TableHead>
                ...
                <TableBody>
                  {transactions.map((transaction) => (
                    <TableRow key={transaction.id} className="border-slate-200 dark:border-slate-700">
                      <TableCell className="font-mono text-sm text-slate-900 dark:text-slate-100">
                        {transaction.id}
                      </TableCell>
                      <TableCell className="text-slate-900 dark:text-slate-100">{transaction.orderId}</TableCell>
                      <TableCell className="text-slate-900 dark:text-slate-100">{transaction.customer}</TableCell>
                      <TableCell className="font-medium text-slate-900 dark:text-slate-100">
                        ${transaction.amount.toFixed(2)}
                      </TableCell>
                      <TableCell className="text-slate-900 dark:text-slate-100">{transaction.method}</TableCell>
                      <TableCell>
                        <Badge className={getStatusColor(transaction.status)}>{transaction.status}</Badge>
                      </TableCell>
                      <TableCell className="text-slate-900 dark:text-slate-100">{transaction.date}</TableCell>
                      <TableCell
                        className={
                          transaction.fee < 0
                            ? "text-red-600"
