...
export function OAuthButtons({ onSuccess, variant = "bordered", className }: OAuthButtonsProps) {
  ...
  const { login } = useUser();
  const { config, loading: authLoading, error } = useAuthConfig();
  ...
  const handleOAuthLogin = async (provider: OAuthProvider) => {
    ...
    const googleEnabled = !!config.googleClientId && config.googleEnabled !== false;
    const facebookEnabled = !!config.facebookClientId && config.facebookEnabled !== false;
    ...
    if (provider === "facebook" && !facebookEnabled) {
      toast.error("Facebook login is not enabled");
  ...
  // While loading and no cached config yet, show skeleton buttons
  if (authLoading && !config && !error) {
    return (
